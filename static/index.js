...
function gameInit() {
    createjs.Sound.registerSound({
        src: "./static/music/NoMoreBraveCowA.mp3",
        id: "NoMoreBraveCowA"
    });
    createjs.Sound.registerSound({
        src: "./static/music/BraveCowBella.mp3",
        id: "end"
    });
    createjs.Sound.registerSound({
        src: "./static/music/thump.mp3",
        id: "tap"
    });
    gameRestart();
}
...
function gameTapEvent(e) {
    ...
    if ((p.id == tar.id && tar.notEmpty) || (p.cell == 0 && x < blockSize) || (p.cell == 1 && x > blockSize && x < 2 *
        ...
        createjs.Sound.play("NoMoreBraveCowA");
        gameOver();
        tar.className += ' bad';
        // 作弊内容
        // if (!_gameStart) {
...
function shareText(score) {
    ...
    SubmitResults();
    if (score <= 99) return '不要勇敢牛牛了，你们刷点别的刷点别的刷点别的！';
    if (score <= 149) return '拒绝勇敢牛牛';
    return '元气满满的A-SOUL舞担参上~目标TOP IDOL，一起加油！';
}
